...
router.get('/post/:postId/comments/', async (req: Request, res: Response, next: NextFunction) => {
  const postId = req.params.postId;
  const postRepo = getConnection().getRepository(Post);
  const comments = await postRepo.findOne(postId, { relations: ['comments'] });
  return res.json({ comments });
});

// Get comment via :commentID
router.get('/comments/:commentId', (req: Request, res: Response, next: NextFunction) => {
...
// Create new comment
router.post('/post/:postId/comments/new', async (req: Request, res: Response, next: NextFunction) => {
  // comments data
